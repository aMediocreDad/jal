	...
	TextEditor._onClickContentLink = async function (event) {
		...
		if (anchor && entity === "JournalEntry") {
			...
			Hooks.once("renderDocumentSheet", (_app, html, _data) => {
				const node = html.find(`:header:contains("${anchor}")`);
				const offset = html.find("form.editable");
				const content = html.find("div.editor-content");
				if (node) {
					content.animate(
						{
							scrollTop:
								node.offset().top - offset.height() / 2 + 20,
						},
						200
					);
					_app.options.scrollY = [node];
				} else
