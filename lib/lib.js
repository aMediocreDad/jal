...
Hooks.on("renderNoteConfig", async (app, html, data) => {
	...
	$(html).find("select[name='entryId']").on("change", _updateSectionList);
	pageid.on("change", _updateSectionList);

	// on change of page section
	function _updateTextLabel() {
		const pagename = app.form.elements["pageId"]?.selectedOptions[0]?.innerText ?? "";
		const sectionname = app.form.elements["flags.anchor.slug"]?.selectedOptions[0]?.innerText ?? "";
		$(html).find("input[name='text']").val(`${pagename}: ${sectionname}`);
	}
	$(html).find("select[name='flags.anchor.slug']").on("change", _updateTextLabel);
...
Hooks.on("dropCanvasData", (_, data) => {
	if (!(data.type === "JournalEntryPage" && data.anchor)) return true;
	const { anchor } = data;

	Hooks.once("renderNoteConfig", (_, html, pagedata) => {
		$(html).find("input[name='text']").val(`${pagedata.pages[pagedata.source.pageId]}: ${anchor.name}`);
